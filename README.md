# Python Backend Interview Preparation

**Pavan Kumar Dubey** — 5.5+ years Python Backend Developer
![Stars](https://img.shields.io/github/stars/pkdubey/python-backend-interview-prep)
Real interview questions & answers based on my resume and experience.

## Modules

| Module | Topic | Questions |
|--------|-------|-----------|
| [Module 1](module-1-hr-resume-project.md) | HR + Resume + Project | Q1–Q20 |
| [Module 2](module-2-python-core.md) | Python Core | Q21–Q40 |
| [Module 3](module-3-django-drf.md) | Django + DRF | Q41–Q60 |
| [Module 4](module-4-mysql-sql.md) | MySQL & SQL | Q61–Q80 |
| [Module 5](module-5-llm-huggingface-celery-aws.md) | LLM, Hugging Face, Celery, Redis, AWS | Q81–Q100 |
| [Module 6](module-6-system-design-coding.md) | System Design + Coding | Q101–Q130 |
| [DSA](dsa-questions.md) | DSA Questions | Q131–Q140 |
| [Practice Code](practice-code.md) | Extra Practice Problems | 10 problems |

## How to Use

1. Har module ko padho
2. Answers ko apne style mein bolo
3. Follow-up questions bhi prepare karo
4. Practice code ko khud se likho
5. Resume ke according hi bolo — fake mat karo

## Final Revision

- AI Moderation Project — Hugging Face/DistilBERT
- Django request flow
- Celery + Redis
- SQL optimization
- Python OOP + decorators + generators
- API design scenarios
- DSA — binary search, linked list, Kadane, two sum, valid parentheses

---

**All the best!**
